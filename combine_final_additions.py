...
infile = 'final_additions.tsv'
inseq = '/home/lakinsm/Documents/phd/phdenv/final_model_qc/todo_files/blast_parsing/data/fixed_full_blast_query.fasta'
inannot = 'hmm_annotations2.csv'
outseq = 'temp_hmm_seqs3.fasta'
outannot = 'temp_hmm_annotations3.csv'
...
with open(infile, 'r') as inf, open(outseq, 'w') as outs, open(outannot, 'w') as outa:
    ...
    for line in inf:
        ...
        if line[1] in db_annot:
            if line[0].split(',').count('') >= db_annot[line[1]].count(''):
                annot = db_annot[line[1]]
            else:
                annot = line[0].split(',')
                annot += [''] * (6 - len(annot))
        else:
